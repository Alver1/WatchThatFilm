...
if (mysqli_num_rows($result) > 0) {
    ...
    if ($row[3] == 1) {
        $sqlupdate = "UPDATE likes SET status = '0' WHERE likes.id_LIKES = '$row[0]'";
        ...
        echo '<script language="javascript">';
        echo 'alert("You have already disliked this film.")';
        echo '</script>';
        $url = htmlspecialchars($_SERVER['HTTP_REFERER']); 
        header("Refresh: 0; '$url'");
    }
    ...
    $sqlinsert = "INSERT INTO likes SET fk_FILMSid_FILMS='$film', fk_USERSid_USERS='$user[0]', status=0";
